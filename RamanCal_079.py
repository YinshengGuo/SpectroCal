...
def print_library_versions():
    ...
    import scipy
    import lmfit
    print('guidata: {}'.format(guidata.__version__),
          'guidata.qt: {}'.format(guidata.qt.__version__),
          'guiqwt: {}'.format(guiqwt.__version__),
...
class interface():
        ...
        if cache.ndim == 2:
            try:  # throws exception if text file format is wrong
                ...
                self.spectrum_range_item = make.range(
                    self.xdata[-1]/2, self.xdata[-1]/2+20)
    ...
    def func_load_ref_wvlen(self):
        ...
        ref_file_name = str(ref_file_name)
...
#        print 'calibration result:\n',slope,intercept,r_value,p_value,std_err
    ...
    def func_calibrate_shift_button(self):
        ...
        p0['cal_Shift_l'].set(value=float(
            self.laser_wavelength.text()), vary=False)
    ...
    def func_save_cal_wavenumber(self):
        # need to have a valid calibration spectrum, use this name with '.cal' extension
        try:
            calibration_file_path = self.spectrum_file_path[:-4] + \
                '_wavenumber.cal'
        except TypeError as err:  # occurs when clicked without loading in spectrum
            message = "TypeError:\t"+err.message
            self.update_status_bar(message)
...
#                                                 QtWidgets.QSizePolicy.MinimumExpanding))
    ...
    def __init__(self):
        ...
        self.load_ref_wvlen_button.setText(
            "Load reference\n Wavelength/\nRaman shift")
